...
export default function SignUpOccupation({navigation}) {
  const {
    university,
    setUniversity,
    course,
  ...
  return (
    <View style={{...globalStyles.screen, ...styles.screen}}>
      ...
      />
      <TextType type="p1" text="2 of 7" textStyle={globalStyles.progressText} />
      ...
      {student ? (
        <SignUpFormStudent navigation={navigation} page={'sign up'} />
      ) : (
        <SignUpFormProfessional navigation={navigation} page={'sign up'} />
      )}
    </View>
  );
}
